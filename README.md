...
- **Skills.** Each `.agents/skills/<name>/SKILL.md` ([Agent Skills](https://agentskills.io) format) at the vault root or inside a folder is a skill. By default the model knows skills by name only: when a task calls for one it finds it with `skill_search` and reads its `SKILL.md`, so descriptions stay out of every request. Set a skill to **Listed** under Tool permissions to show its description upfront instead. `/skill <name>` runs one directly.
